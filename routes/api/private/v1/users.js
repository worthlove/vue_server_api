...
router.delete("/batch",
    ...
    function (req, res, next) {
        if (!req.body.ids || !Array.isArray(req.body.ids)) {
            return res.sendResult(null, 400, "用户ID列表不能为空且必须是数组");
        }
    ...
    // 处理业务逻辑
    function (req, res, next) {
        // 使用Promise.all来处理多个删除操作
        const deletePromises = req.body.ids.map(id => {
            return new Promise((resolve, reject) => {
                mgrServ.deleteManager(id, function (err) {
                    if (err) reject(err);
                    else resolve();
                })(req, res, next);
            });
        ...
        Promise.all(deletePromises)
            .then(() => {
                return res.sendResult(null, 200, "批量删除成功");
            })
            .catch(err => {
                return res.sendResult(null, 400, "批量删除失败：" + err);
            });
    }
);
